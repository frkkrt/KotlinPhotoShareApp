...
class Photo_Share_Activity : AppCompatActivity() {
    ...
    fun paylas(view: View)
    {
        ...
        {
            gorselReferance.putFile(secilenGorsel!!).addOnSuccessListener { success->
                val yuklelenenGorselReference=FirebaseStorage.getInstance().reference.child("images").child(gorselIsmi)
                yuklelenenGorselReference.downloadUrl.addOnSuccessListener {
                    val downloadUrl=it.toString()
                    val guncelKullaniciEmail=auth.currentUser!!.email.toString()
                    val kullaniciYorumu=yorumText.text.toString()
                    //güncel tarihi detayları ile alır.
